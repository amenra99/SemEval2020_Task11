...
logger = logging.getLogger('propaganda_train_SI_BERT_base')
...
def trainBert(train_x, train_y, val_x, val_y):
    checkpoint_path = "model-SI-BERT/cp-base-{epoch:04d}.ckpt"
...
def evaluate(modelPath, train_x, train_y, val_x, val_y, num=EPOCHS):
    ...
    for i in range (1, num + 1):
        ...
        # Train score
        tr_pred, tr_labels = getPredGold(bertModel, train_x, train_y, "npy/SI-train-BERT-base-result-{0:04d}.npy".format(i))
        tr_f1, tr_precision, tr_recall = calculateScore(tr_pred, tr_labels)

        # Val score
        val_pred, val_labels = getPredGold(bertModel, val_x, val_y, "npy/SI-val-BERT-base-result-{0:04d}.npy".format(i))
