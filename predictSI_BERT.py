...
from transformers import BertConfig, BertTokenizer, TFBertForTokenClassification, TFBertForSequenceClassification
...
logger = logging.getLogger('propaganda_train_SI')
        ...
        for sent in sents:
            ...
            for i, token in enumerate(tokens):
                ...
                if token in ['[CLS]', '[UNK]']:
                    ...
                    # if start == -1:
                    #     print("{0}\t{1} : [{2}] - {3}\t{4}".format(key, (start, end), token, sent[:end], sent))
                    #     print(tokens)
                    #     print('\n')
                    assert start > -1, "{0}\t{1} : [{2}] - {3}\t{4}".format(key, (start, end), token, sent[:end], sent)
...
def buildData(article_dir, label_file):    
    ...
    return x, token_spans


def getBertModel():
    config.num_labels = 2
    bertModel = TFBertForTokenClassification.from_pretrained(PRETRAINED_MODEL, config=config)
...
def predictAll(modelPath, x, num=10):
    ...
    for i in range(1, num + 1):
        ...
        npyFile = 'npy/SI-val-BERT-base-result-{0:02d}.npy'.format(i)
...
def predict(modelPath, x):
    ...
    result = bertModel.predict(x, batch_size=BATCH_SIZE)
    npyFile = 'npy/SI-test-base-BERT-result.npy'
    np.save(npyFile, result)
    print(npyFile + " saved")
    pred = np.argmax(result, axis=-1)
...
def main(mode='multi', model='npy', num=10):
    ...
    checkpoint_path = "model-SI-BERT/cp-base-{0:04d}.ckpt"
    submissionFile = 'submission_SI/submission_SI_base_BERT_{0}.txt'
    npy_path = "npy/SI-val-BERT-base-result-{0:04d}.npy"

    test_x, token_spans = buildData(dev_article_dir, dev_label_file)
    ...
    if mode == 'multi':
        ...
        for i, pred in enumerate(preds):
            ...
            writeSubmissionFile(submissionFile.format(i + 1), article_span)
        ...
        if model == 'npy':
            ...
            pred = predict(checkpoint_path.format(14), test_x)

        article_span = reconstructSpans(token_spans, pred)
        writeSubmissionFile(submissionFile.format(0), article_span)
